...
- 완료 기준 충족 여부(criteriaMet)는 지시문의 "완료 기준" 섹션 각 항목을 코드가 실제로 만족하는지로만 판단한다. 미충족이면 unmetReasons에 어떤 기준이 부족한지만 짧게 적고, 고치는 방법이나 정답은 적지 않는다 — 그건 feedback 필드의 예측 질문 방식으로 유도한다.
