...
- 시작 코드 안의 주석(\`//\`, \`/* */\`)은 한글이 아니라 영어로 쓴다. Sandpack의 런타임 에러 화면이 코드 프레임을 보여줄 때 한글 주석 근처에서 문자가 깨지는 인코딩 버그가 있다(학습자가 TODO 주석 자리를 채우다 에러를 내는 경우가 많아 특히 자주 노출된다). 화면에 렌더링되는 문자열(JSX 텍스트, UI 라벨)은 이 문제와 무관하므로 계속 한국어로 써도 된다.
