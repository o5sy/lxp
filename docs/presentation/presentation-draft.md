...
- **(2026-08-26 변경)** 다른 컴퓨터에서 확실히 이어받기 위해 최종본과 원본을 `docs/video/`에 git으로 커밋해뒀다(`demo-final.mp4`, `demo-raw-original.mov`) — repo가 ~68MB 무거워지는 트레이드오프를 감수. 발표 당일엔 여기서 받아 슬라이드 소프트웨어(Keynote/PowerPoint/Google Slides)에 직접 삽입해서 쓴다.
