# 발표 슬라이드 (최종, 발표 완료) — AI 개발 멘토

**발표 시간:** 총 7분 (발표 5분 + 질의응답 2분) · **슬라이드 구성:** 5장
**발표일:** 2026-08-26 · **참고 문서:** [원페이저](designs/onepager-submission.md) · [PRD](designs/ai-adaptive-practice-generator.md) · [ADR](adr/README.md)

*이 문서는 실제 발표에 쓰인 [Notion 발표 스크립트](https://app.notion.com/p/o5sy/AI-3c8d9c31c75781b5a90bd786b7871fbe) 최종본을 기준으로 갱신했다 — 스피커 노트는 Notion과 동일하게 유지하고, 촬영 가이드·ffmpeg 편집 기록·Q&A 대비 배경 메모 등 제작 상세는 이 문서에만 남긴다(Notion에서도 이 문서를 원본 상세 문서로 지목함).*
...
| S3. 사용 기술 및 선정 이유 | 30~40초 | 2:20 – 2:55 |
...
> 안녕하세요, 개발 직군 오승연 입니다. 1차 프로젝트로 만든 '개발 학습 AI 튜터'를 소개해드리겠습니다.

## 목차

> 주제는 프론트엔드 개발 직무 프로젝트로, AI 기능이 들어간 프로덕트 화면 하나를 완성하는 것이었습니다.
> 발표 순서는 다음과 같습니다.
...
- 이탈 원인은 매번 대화를 주고받으며 실습을 만들고 환경도 직접 구성해야 하는 **번거로움과 모호함**
...
> 저는 맞춤형 개발 실습을 만들고 리뷰해주는 AI 튜터 서비스를 만들었습니다. 계기는 개발 멘토링 경험이었습니다 — 수강생 대부분이 개념 강의를 들어도 실습이나 과제엔 손을 못 대는 걸 지켜봤고, 실제로 들여다보니 배운 개념만으로는 진행하기 어려운 경우가 많았고 힌트가 있어도 수강생 별로 이해도 차이가 컸습니다. 그래서 개개인의 수준에 맞춘 실습의 필요성을 느꼈습니다. AI 활용 학습법을 가이드하고 Gemini Gems라고 클로드 프로젝트와 같이 재사용할 수 있는 지침을 만들어 공유했는데 — 시도한 건 4~5명 중 1~2명뿐이었고, 그마저 2주 안에 이탈했습니다. 이유를 들어보니 매번 대화를 주고받으며 실습을 만들고, 환경 또한 직접 구성해야 하는 번거로움과 모호함 때문이었습니다. 그래서 프롬프트 작성을 구조화된 선택 UI로 대체하고, 같은 화면에서 실행까지 끝낼 수 있도록 문제 해결의 방향성을 잡았습니다.

**말하기 메모:** "4~5명 중 1~2명"과 "2주 내 이탈"이 이 슬라이드의 유일한 숫자 근거이니 또박또박 말한다. 4단계 학습법론 세부 내용은 시간상 생략하고 바로 다음(데모)으로 넘어간다.
...
1. **개념 입력 (~17초)** — "배웠지만 아직 코드로 적용해보지 못한 개념을 입력합니다. 자주 나오는 개념은 자동완성으로 제안되고, Tab 키만으로도 바로 선택할 수 있어서 키보드만으로 조작할 수 있습니다."
2. **목표 선택 (~17초)** — "다음은 이번 실습에서 뭘 목표로 할지 고릅니다. 문법이 헷갈리는지, 코드로 적용하는 게 어려운지, 여러 개념을 종합해서 쓰는 게 어려운지 본인 상황에 필요한 단계를 고릅니다. (마우스 없이 키보드만으로도 잘 조작할 수 있습니다.)"
3. **추가 설명, 선택 (~10초)** — "더 구체적으로 학습하고 싶은 부분이 있으면 한 줄 적을 수 있는데, 비워두고 바로 넘어가도 됩니다. 여기까지 전부 클릭(또는 선택적 한 줄 입력)만으로 끝나고, 프롬프트를 직접 작성하는 흐름은 어디에도 없습니다. 프롬프트 작성에 대한 부담과 매번 지침을 복사하는 번거로움을 낮춥니다."
4. **생성 → 스트리밍 (~20초)** — "생성을 누르면 이 값들을 조합해 실습 지시문과 시작 코드를 생성하는 프롬프트를 만들고, 응답이 오면 토큰 단위로 스트리밍됩니다. 이 동안 에디터는 read-only로 잠겨 있어요."
5. **실습 진행 (~20초, 영상에서는 5배속 편집)** — "스트리밍이 끝나면 에디터가 편집 가능한 상태로 바뀌고, 같은 화면 안에서 바로 코드를 작성하고 실행할 수 있습니다. 과거에는 AI 데스크탑 앱이랑 코드 에디터를 따로 띄워서 쓰는 방식이었는데, 한 화면에서 진행하면서 그 전환 비용을 제거했습니다. 지금은 지시 확인부터 실행, 피드백까지 한 화면에서 가능합니다. (이 구간은 실제로 더 오래 걸려서 영상에서는 5배속으로 편집했습니다.)"
6. **[피드백 받기] → 피드백 (~20초)** — "코드를 수정한 뒤 피드백 받기를 누르면 AI가 피드백을 줍니다. 정답을 바로 알려주지 않고, 관찰한 현상을 설명해주고 다음 질문을 던지는 톤으로 만들었습니다."
7. **완료 판정 (~10초)** — "마지막으로 완료 기준 각 항목의 충족 여부를 판정해서, 다 채웠으면 완료 기준 충족, 아직이면 부족한 부분이 있다는 걸 보여주며 루프를 닫습니다."

**말하기 메모:** Sandpack이 오픈소스 코드샌드박스 라이브러리라는 설명은 라이브 대본에 넣지 않는다 — S3에서 이미 다루므로 중복이고, 5번 스텝 하나에 다 넣으면 시간 예산의 절반을 써버린다. 대신 영상 자막에 넣어뒀고, Sandpack 언급은 Q&A "그 코드 에디터는 뭘로 만들었냐" 질문 대비용으로 남겨둔다.

**진행 메모 (발표 시 읽지 않음):** 라이브 데모가 중간에 막히면(스트리밍 지연, API 실패 등) 즉시 녹화 백업 영상("시연 영상" 슬라이드)으로 전환한다. 어느 지점에서 끊겨도 자연스럽게 이어받을 수 있도록 녹화본도 동일한 7단계 순서로 준비해둔다.
...
> 설계 결정은 슬라이드 순서대로 말씀드리겠습니다. 먼저 Next.js는, GEMINI API 키를 클라이언트에 노출하지 않고 서버를 거쳐 안전하게 호출하는 기능을 짧은 기간 안에 구현하기 위해 선택했습니다. 코드 편집·실행 화면은 Sandpack을 사용했습니다 — 다른 오픈소스도 검토했지만 대부분 실행 환경 자체를 직접 구축해야 하는 방식이라, 환경까지 갖춰진 Sandpack이 정해진 기간 안에 작업하기에 더 맞았습니다. 실습 지시문·코드·피드백을 만드는 AI 기능은 Gemini API로 연동했고, 주요 AI 호출에 대한 표준 인터페이스와 스트리밍을 지원하는 Vercel AI SDK를 함께 썼습니다. GEMINI는 무료 사용량으로 먼저 검증해보려고 골랐는데 품질도 충분히 괜찮았습니다. 이후 고도화할 때 로컬에 무료 모델로 변경해볼까도 싶어서 AI 호출부는 추상화하라고 지시했습니다. 상태 관리는 평소엔 Context API를 주로 쓰는데, 이번엔 토큰 단위로 계속 스트리밍되다 보니 관계없는 컴포넌트까지 리렌더되면서 성능 문제가 발생할 수 있고, Context로 이를 막으려면 템플릿 코드가 많이 필요해서, 필요한 값만 구독하는 Zustand를 선택했습니다. 마지막으로 접근성은 ARIA 속성이나 키보드 포커스 같은 부분을 일일히 구현하지 않아도 기본은 챙길 수 있도록 Base UI 라이브러리 위에 디자인을 입혀서 사용했습니다.
...
> 검증은 직접 하기에 시간이 부족해서 성능만 클로드 코드에게 백그라운드로 Lighthouse 측정과 리포트 정리를 지시했습니다 — 그 결과 메인 페이지 JS는 163KB로 가볍고, 성능 75점, 접근성 94점이 나왔습니다. 폰트 서브셋 때문에 커진 번들 크기를 줄이고, 평균 13초대인 스트리밍 첫 응답 체감 속도 개선, 상태와 요구사항이 복잡해져도 안정적으로 확장 가능한 구조를 만드는 데 집중할 계획입니다.

**말하기 메모:** "개선 과제로 남겼다"는 말을 회피하듯 넘기지 말고 또박또박 — 이 발표의 정직성 포인트다. 질문이 들어오면 "왜 못 했나"보다 "5일 스코프에서 무엇을 먼저 챙겼는가"로 답한다(PRD Day2 컷라인: 5개 UI 상태·이상입력방어·재현성·성능측정·문서화는 반드시 사수). 숫자가 많은 슬라이드라 말은 슬라이드를 짚어가며 천천히 한다.
...
- 소감: 짧은 기간 안에 이 정도 구현이 가능하다는 데 놀랐고, 구현은 AI에 위임하고 문제 정의·솔루션 설계에 집중할 수 있었음
  → 다음은 **폰트 서브셋 축소로 번들 감량 · 스트리밍 첫 응답(TTFT) 체감 속도 개선 · 상태·요구사항 변화에 강한 아키텍처 · 성능·보안 보완**을 기반으로 다질 예정

### 스피커 노트 [3:50–4:35]

> 이번 프로젝트의 목표는 AI 기능이 들어간 화면 하나를 완성도 있게 만드는 것이었습니다. 멘토링 현장에서 직접 관찰한 문제와 데이터를 바탕으로, 학습 단계에 따라 실습 목표가 달라야 한다는 필요성을 느꼈었는데, 이를 실제 제품 기능으로 만들었다는 점에서 의미가 있었습니다. 짧은 기간 안에 이 정도 완성도까지 구현이 가능했다는 점이 놀라웠고, 구현은 AI에게 위임하고 저는 문제 정의와 핵심 솔루션 설계에 집중할 수 있었던 게 가장 좋았던 경험이었습니다. 앞으로 좀 더 완성도있게 확장하고자 합니다. 개선할 점이 많이 남아있기도 하고, 좀 더 개발자 관점에서 성능과 보안을 신경 쓰고자 합니다. 예를 들어 실측한 내용 중 폰트 서브셋 때문에 커진 번들 크기를 줄이고, 평균 13초대인 스트리밍 첫 응답 체감 속도 개선, 상태와 요구사항이 복잡해져도 안정적으로 확장 가능한 구조를 만드는 데 집중할 계획입니다.

**말하기 메모:** 마지막 문장("감사합니다")까지 포함해 4:35 안에 끝내는 걸 목표로 하되, 마무리 슬라이드이니 조금 늘어져도 5:00 안에만 들어오면 된다 — 여유 25초는 여기서 써도 무방하다.

---

## 감사합니다 · Q&A

> 이상으로 발표를 마치겠습니다. 질문 있으시면 편하게 해주세요. 감사합니다.
...
- [x] **발표 완료 (2026-08-26)** — 위 스피커 노트는 Notion 발표 스크립트 최종본 기준으로 동기화됨. 남은 미체크 항목(라이브 리허설·전체 타이밍 측정)은 발표가 이미 끝나 더 이상 유효하지 않은 준비 항목으로, 과거 준비 상태 기록으로만 남겨둔다.
