...
부트캠프 평가 배점(문제정의15·동작완성도20·성능20·안정성15·재현성15·문서15)에 맞춰 우선순위를 정했다. **실측치가 없는 항목은 추정치로 채우지 않고 "개선 과제"로 정직하게 남겼다** — 이 자체가 검증 전략상의 의사결정이다.
...
**기술 컨텍스트:** 프론트엔드→백엔드는 REST/JSON 요청, 백엔드→프론트엔드는 SSE(Server-Sent Events) 스트리밍 응답. 백엔드→Gemini는 Vercel AI SDK(`streamObject`)를 통한 스트리밍 호출. LLM 접근 토큰은 서버 환경변수에만 존재하며 클라이언트 번들에는 절대 노출하지 않는다([misc.md](adr/misc.md)).
