...
- **실습 세션 화면(`/practice/[id]`):** 전체 화면(edge-to-edge, 여백 없음) 4분할. traffic-light 창 크롬은 **페이지 전체에 딱 하나**(최상단 타이틀바)만 두고, 각 패널은 창처럼 보이지 않게(닫기/이동 가능해 보이는 UI가 실제로는 그렇지 않아 교육 자료로 쓸 때 혼란을 준다는 피드백) 얇은 `border-line` 구분선 + 소문자 라벨(mono, uppercase, faint)만으로 영역을 나눈다. 배치: 좌측 지시문(전체 높이) / 우측은 위쪽 에디터(크게) + 아래쪽 AI 피드백·코드 확인 버튼(각각 별도 패널, 고정 높이 행).
