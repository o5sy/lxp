...
- **진행률(스텝 레일):** 프롬프트 빌더 터미널 창 **바깥, 페이지 상단**에 배치 (창 내부가 아님). 상단 줄은 `# STEP N OF 4 — <스텝명>`(주석 컨셉 유지) / `N%`를 space-between으로 배치하고, 그 아래 사각 노드(모서리만 둥글게) + 연결선으로 된 스텝 레일. 완료 노드는 체크(✓) 채움, 현재 노드는 같은 색에 링(box-shadow)으로 강조, 대기 노드는 테두리만. 노드·연결선 색은 accent blue 하나로 통일(success green 미사용 — green은 정답/통과 등 실제 성공 시맨틱 전용으로 남겨둠).
