...
- **실습 세션 화면(`/practice/[id]`):** 3영역(지시문 / Sandpack 에디터+프리뷰 / AI 피드백 패널) 모두 macOS 터미널 창(traffic-light 점 + 타이틀바) 사용 — 실제 프로그램 화면이라 창 프레이밍이 자연스러움.
...
- **Max content width:** 프롬프트 빌더 640px 전후(단일 컬럼 폼), 실습 세션은 980px(프리뷰 기준) 3영역 그리드에 맞춰 조정.
